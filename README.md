...
**現状**: **Tello EDU で外部カメラによる自動ホバリングと経路追従が成立**
（静止時 平均3.4cm、20cm四方の正方形を一周して各点 平均3.5cm）。StampFly は自動ホバリング未達成のまま中断している。
詳しい経緯と手順は [HANDOFF.md](HANDOFF.md)、原因調査の記録は
...
| `tello/` | **Tello EDU**（外部カメラによる自動ホバリングが成立した機体） |
...
| ファイル | 用途 |
|---|---|
| `hover_oakd.py` | **本命**。OAK-D で測った位置で離陸地点の真上に留める（P+I 制御） |
| `hover_test.py` | Tello 単体のホバリングの安定度を測る（外部制御の土台の確認） |
| `check.py` | 飛ばさずに状態を確認する |
| `place.py` | 経路が画面に収まる置き場所をカメラで測って求める（飛ばさない） |
| `plot.py` | 飛行のログを図にする（軌跡・ずれ・外乱の打ち消し量） |
...
Tello EDU で飛ばす手順は [HANDOFF.md](HANDOFF.md) の「7. Tello EDU での自動
ホバリング」にまとめてある。以下は StampFly の手順。
