...
`--waypoints` で離陸地点からのずれを並べ、`--dwell` 秒ごとに次の点へ移る。
30cm の往復で **平均ずれ 5.4cm、各点の後半6秒で 平均2.3cm・最大4.9cm**、
行き過ぎ 3.4cm、到達（5cm以内）まで 2.9秒。

```bash
python3 tello/hover_oakd.py --waypoints "0,0 0.3,0 0,0" --dwell 12 --probe-dist 0.15 \
    --log ~/Desktop/tello_path.csv --fence 0.6
```

制御は **P + I + D**。それぞれが別の症状に効く（[docs/findings.md](docs/findings.md) 14〜16番）。

| 項 | 既定 | これが無いと |
|---|---|---|
| P `--kp` | 1.0 | 動かない |
| I `--ki` | 0.35 | 一定のずれが残る（実測 -7cm） |
| D `--kd` | 0.6 | 行き過ぎて振動が止まらない（±30cm） |
| 目標の移動 `--move-speed` | 0.15 m/s | 一瞬で飛ばすと行き過ぎる |
| 積分の範囲 `--i-band` | 0.10 m | 移動中に溜まって押し出す |

### 置き場所の決め方

画角に収まらないと離陸しない。収まる場所は計算できる。

```bash
python3 tello/place.py --waypoints "0,0 0.3,0 0,0" --probe-dist 0.15
```

縦方向は向きの測定（前後 `--probe-dist`）だけが制約になる。カメラを 2m まで
下げた状態で、測定15cm なら経路は **前へ62cm** まで伸ばせる。測定20cm は縦が
ぎりぎりで余裕が2cmしかないので、15cm を使う。

### 正方形（同日・達成）

20cm 四方を一周（5点・各12秒・60秒）。**落ち着いたあとの精度 平均3.5cm・最大7.5cm**。
y 方向も正しく制御でき、向きの換算が両軸で正しいことを確認した。

```bash
python3 tello/hover_oakd.py --waypoints "0,0 0.2,0 0.2,0.2 0,0.2 0,0" --dwell 12 \
    --probe-dist 0.15 --log ~/Desktop/tello_square.csv --fence 0.6
```

| 点 | 指示 | 落ち着いた位置 | 後半6秒のずれ |
|---|---|---|---|
| 1 | (0, 0) | (0.0, 0.0) | 3.5cm |
| 2 | (20, 0) | (18.9, 0.5) | 1.7cm |
| 3 | (20, 20) | (16.0, 21.1) | 2.9cm |
| 4 | (0, 20) | (-5.0, 21.8) | 4.4cm |
| 5 | (0, 0) | (0.8, -4.3) | 5.1cm |

**後半の点ほどずれが大きい**（1.7→2.9→4.4→5.1cm）。各点の後半6秒でも改善が
続いているので、12秒では積分が効き切っていない。`--dwell 15` で角の精度は上がるはず。

機体が勝手に流される速さの実測は **0.9〜2.0cm/s**。定常偏差 -7cm の原因として
つじつまが合う。

### 長方形 40x15cm（同日）

正方形は画角の都合で 20cm 四方が限界なので、横長にして移動距離を伸ばした。
**40cm 動かしても行き過ぎは 4.8〜7.2cm**（20cm のときが 3.4cm）で、微分項が
距離によらず効くことを確認した。

| 回 | 落ち着いたあと | 備考 |
|---|---|---|
| rect  | 平均5.2cm 最大11.1cm | 積分の上限10で頭打ち |
| rect2 | 平均3.8cm 最大 9.5cm | 上限を18に。4点目1.6cm、5点目3.3cm |
| rect3 | 平均9.2cm 最大48.8cm | **外乱が強い回**（流れ17.8cm/s）。下記 |

### 道具

| コマンド | 用途 |
|---|---|
| `python3 tello/place.py --waypoints "..." --probe-dist 0.15` | 収まる置き場所、または経路の上限を求める |
| `python3 tello/plot.py ~/Desktop/xxx.csv` | 軌跡・ずれ・外乱の打ち消し量を1枚の図にする |

数字の表だけでは、行き過ぎているのか流されているのかが分かりにくい。
**まず `plot.py` で図にしてから考えること。**

### 未解決: 外乱の強さが日によって大きく変わる

向きの測定で出る「流れ」が、**0.9〜2.0cm/s の回もあれば 17.8cm/s の回もある**。
強い回は制御が押し返しきれず、平均ずれが 3.8cm → 9.2cm に悪化する（設定は同じ）。

切り分けは `hover_test.py` が早い。機体単体の水平速度が

- **0.0cm/s のまま** → 環境ではなく制御側を疑う
- **5cm/s 以上** → 機体が自分で位置を保てていない。風か、**床の模様**
  （Tello は下向きカメラで位置を保つので、模様のない床・反射する床・暗い床では
  保持が効かない。新聞紙やマットを敷くと改善することがある）

### 次にやること
- 上の切り分け（`hover_test.py` で単体の安定度を測り直す）
- `--dwell 15` で角の精度が上がるか（12秒では積分が効き切っていない）
- **カメラをもっと下げる。** いまの画角では正方形は 20cm 四方が限界
  （縦方向は向きの測定 ±15cm だけで帯を使い切る）。30cm 以上をやるなら必要
- マーカーを 6cm に印刷し直す（4cm では高さの測定が置き場所ごとに10cmばらつく）
- 高さも PC で制御する（いまは Tello 任せ）
